...
pub fn requirements_table_view(graph: &Graph) -> RequirementTableViewDto {
    let derived = mercurio_foundation::materialize_core_indexes(graph, &[]).ok();
    let mut rows = graph
        .elements()
        .iter()
...
fn artifact_digest(value: &Value) -> String {
    ...
    mercurio_foundation::stable_digest([("requirement-analysis".as_bytes(), bytes.as_slice())])
}

pub fn explain_semantic_goal(goal: &SemanticGoalSpec) -> SemanticGoalExplanation {
    mercurio_foundation::explain_semantic_goal(goal)
}
...
pub fn evaluate_semantic_goal(
    ...
    mercurio_foundation::evaluate_semantic_goal(context, goal)
}

pub fn default_model_quality_profile() -> SemanticGoalProfile {
    mercurio_foundation::default_model_quality_profile()
}
...
fn derived_sources(
    derived: &Option<mercurio_foundation::DerivedIndexes>,
    requirement_id: &str,
    relation: &str,
) -> Option<Vec<String>> {
...
fn is_requirement(element: &mercurio_foundation::graph::Element) -> bool {
...
fn is_library_requirement(element: &mercurio_foundation::graph::Element) -> bool {
    element.element_id.contains("::")
}

fn is_requirement_relationship(element: &mercurio_foundation::graph::Element) -> bool {
...
fn related_sources(
    graph: &Graph,
    requirement: &mercurio_foundation::graph::Element,
    relations: &[&str],
) -> Vec<String> {
...
fn is_relationship_element(
    element: &mercurio_foundation::graph::Element,
    relations: &[&str],
) -> bool {
    let kind = element.kind.to_ascii_lowercase();
    relations
...
fn source_for(element: &mercurio_foundation::graph::Element) -> Option<RequirementSourceDto> {
...
fn string_property(element: &mercurio_foundation::graph::Element, key: &str) -> Option<String> {
    element
        .properties
        .get(key)
        .and_then(Value::as_str)
...
mod tests {
    use super::*;
    use mercurio_foundation::KirElement;
