...
const PROJECT_DESCRIPTOR_FILE_NAME: &str = "mercurio.project.json";
...
struct ProjectDescriptor {
    ...
    name: Option<String>,
    #[serde(default)]
    libraries: Vec<ProjectLibraryConfig>,
    #[serde(default)]
    plugins: Vec<Value>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct ProjectLibraryConfig {
    id: String,
    #[serde(default)]
    role: Option<String>,
    #[serde(default)]
    provider: LibraryProviderConfig,
}
    ...
    fn from_path(path: &Path) -> Result<Self, CliError> {
        let content = std::fs::read_to_string(path).map_err(|err| {
            CliError::execution(format!(
                "failed to read project descriptor {}: {err}",
                path.display()
            ))
        })?;
        serde_json::from_str(&content).map_err(|err| {
            CliError::execution(format!(
                "failed to parse project descriptor {}: {err}",
                path.display()
            ))
        })
    }
...
struct LintDiagnostic {
    ...
    span: Option<mercurio_core::frontend::ast::SourceSpan>,
}
...
fn run_lint(command: LintCommand) -> Result<RunResult, CliError> {
    ...
    let reports = sources
        .iter()
        .map(|source| lint_source(source, &context_modules, &library_context.document))
        .collect::<Vec<_>>();
...
fn run_package_build(command: PackageBuildCommand) -> Result<RunResult, CliError> {
    ...
    let precompiled_kir = match (precompiled_kir, sources.is_empty()) {
        (Some(document), _) => Some(document),
        (None, false) => Some(compile_package_sources(
            &sources,
...
fn compile_package_sources(
    sources: &[KparPackageSource],
    stdlib: &KirDocument,
) -> Result<KirDocument, CliError> {
    let mut documents = Vec::new();
    for source in sources {
        let language = SourceLanguage::from_path(Path::new(&source.path)).ok_or_else(|| {
            CliError::usage(format!(
                "unsupported package source extension: {}",
                source.path
            ))
        })?;
        let input = SourceInput {
            source_name: source.path.clone(),
            language,
            content: source.content.clone(),
        };
        let response = compile_source(&input, stdlib);
        match (response.status, response.document) {
            ...
            _ => {
                let diagnostics = response
                    ...
                    .join("; ");
                return Err(CliError::execution(format!(
                    "failed to compile package source {}: {}",
                    source.path,
    ...
    KirDocument::merge(documents)
        .map_err(|err| CliError::execution(format!("failed to merge package KIR: {err}")))
}

fn lint_source(
    source: &SourceInput,
    _context_modules: &[SysmlModule],
    library_context: &KirDocument,
) -> LintReport {
    let registry = default_language_registry();
    let language_id = source.language.as_str();
    let diagnostics = match registry.service_for_language(language_id) {
        Some(service) => {
        ...
        None => vec![LintDiagnostic {
            ...
            message: format!("no registered language service for `{language_id}`"),
            span: None,
        }],
    };
...
fn load_library_context(
    ...
    if let Some(descriptor_path) = find_project_descriptor(&open_path) {
        return load_project_descriptor_context(&descriptor_path).map_err(|err| {
            CliError::execution(format!(
    ...
    let document = match default_language.unwrap_or(SourceLanguage::Sysml) {
        SourceLanguage::Sysml => load_sysml_baseline()
            .map_err(|err| CliError::execution(format!("failed to load SysML baseline: {err}")))?,
        SourceLanguage::Kerml => mercurio_kerml::load_kernel_baseline()
            .map_err(|err| CliError::execution(format!("failed to load KerML baseline: {err}")))?,
    };
    Ok(LibraryContext {
        document,
...
fn find_project_descriptor(open_path: &Path) -> Option<PathBuf> {
    let start = if open_path.is_dir() {
        ...
        open_path.parent().unwrap_or(open_path)
    };
    start
        .ancestors()
...
fn load_project_descriptor_context(descriptor_path: &Path) -> Result<LibraryContext, CliError> {
    let descriptor = ProjectDescriptor::from_path(descriptor_path)?;
    let base_dir = descriptor_path.parent().unwrap_or_else(|| Path::new("."));
    if let Some(library) = descriptor.libraries.first() {
        let artifact = library
            .provider
            .resolve_from(&library.id, Some(base_dir))
            .map_err(|err| {
                CliError::execution(format!(
                    "failed to resolve library `{}` from {}: {err}",
                    library.id,
                    descriptor_path.display()
                ))
            })?;
        return Ok(LibraryContext {
            document: artifact.document,
            project_descriptor: ProjectDescriptorUsage::Used(descriptor_path.to_path_buf()),
        });
    }

    Ok(LibraryContext {
        document: load_sysml_baseline()
            .map_err(|err| CliError::execution(format!("failed to load SysML baseline: {err}")))?,
        project_descriptor: ProjectDescriptorUsage::Used(descriptor_path.to_path_buf()),
    })
}
...
fn count_declaration<'a>(declaration: &'a Declaration, counts: &mut BTreeMap<&'static str, usize>) {
    if let Some(definition) = declaration.as_definition_like() {
        ...
        let key = match definition.keyword.as_str() {
            "part" => "part definitions",
            _ => "generic definitions",
        };
    ...
    if let Some(usage) = declaration.as_usage_like() {
        ...
        let key = match usage.keyword.as_str() {
            "part" => "part usages",
            _ => "generic usages",
        };
        *counts.entry(key).or_insert(0) += 1;
...
mod tests {
  ...
  "services": [
    ...
    fn package_verify_checks_bundled_stdlib_package() {
        let root = temp_dir("mercurio-cli-package-verify-stdlib");
        std::fs::create_dir_all(&root).unwrap();
        let kir_path = root.join("stdlib.kir.json");
        let package_path = root.join("stdlib.kpar");
        ...
        write_kpar_package(&package_path, &package).unwrap();
        LocalPackageRepository::new(&repo_path)
            .stage_kpar(&package_path, "org.omg/sysml-stdlib", "2.0.0", None)
            .unwrap();
  ...
  "libraries": [
    {
      ...
      "provider": {
        ...
        .unwrap();
        let stdlib = mercurio_sysml::default_sysml_library_path();
